"""Tests for shared/pole_raw_data_loader.py"""
...
from shared import pole_raw_data_loader
...
class TestCapitalizeKey:
    def test_camel_case_becomes_pascal_case(self):
        assert pole_raw_data_loader._capitalize_key("lastUpload") == "LastUpload"
        assert pole_raw_data_loader._capitalize_key("productName") == "ProductName"
        assert pole_raw_data_loader._capitalize_key("batteryVoltage1") == "BatteryVoltage1"
    ...
    def test_does_not_lowercase_the_rest_of_the_string(self):
        ...
        character.
        """
        assert pole_raw_data_loader._capitalize_key("lastUpload") != "lastUpload".capitalize()

    def test_empty_string_is_unchanged(self):
        assert pole_raw_data_loader._capitalize_key("") == ""
...
class TestParseIsoDatetime:
    def test_none_returns_none(self):
        assert pole_raw_data_loader._parse_iso_datetime(None) is None

    def test_empty_string_returns_none(self):
        assert pole_raw_data_loader._parse_iso_datetime("") is None

    def test_confirmed_leadsun_format_parses(self):
        """Exact format confirmed from a real Leadsun response."""
        result = pole_raw_data_loader._parse_iso_datetime("2026-07-15T12:35:30.000+00:00")
        assert result == "2026-07-15 12:35:30.000 +00:00"

    def test_z_suffixed_utc_string_parses(self):
        result = pole_raw_data_loader._parse_iso_datetime("2026-07-02T18:00:00.000Z")
        assert DTO_PATTERN.match(result)

    def test_garbage_string_returns_none(self):
        assert pole_raw_data_loader._parse_iso_datetime("not-a-date") is None

    def test_unexpected_type_returns_none_instead_of_raising(self):
        assert pole_raw_data_loader._parse_iso_datetime(object()) is None
...
class TestMapLampRecord:
    def test_product_name_renamed_to_location_id(self, make_lamp_record):
        ...
        result = pole_raw_data_loader._map_lamp_record(record)
    ...
    def test_missing_last_upload_gets_sentinel_not_none(self, make_lamp_record):
        ...
        result = pole_raw_data_loader._map_lamp_record(record)
        assert result["LastUpload"] == pole_raw_data_loader._MISSING_LAST_UPLOAD_SENTINEL
    ...
    def test_sentinel_is_stable_across_repeated_calls(self, make_lamp_record):
        ...
        """
        record = make_lamp_record(last_upload=None)
        first = pole_raw_data_loader._map_lamp_record(record)["LastUpload"]
        second = pole_raw_data_loader._map_lamp_record(record)["LastUpload"]
        assert first == second
...
class TestBuildRow:
    def test_row_length_matches_all_columns(self, make_lamp_record):
        mapped = pole_raw_data_loader._map_lamp_record(make_lamp_record())
        row = pole_raw_data_loader._build_row(mapped, sp_exec_id=42)
        assert len(row) == len(pole_raw_data_loader._ALL_COLUMNS)

    def test_row_order_matches_all_columns(self, make_lamp_record):
        mapped = pole_raw_data_loader._map_lamp_record(make_lamp_record(product_name="LOC-X"))
        row = pole_raw_data_loader._build_row(mapped, sp_exec_id=99)

        as_dict = dict(zip(pole_raw_data_loader._ALL_COLUMNS, row))
...
class TestStagingMergeSqlStructure:
    def test_staging_table_ddl_has_guard_and_matches_all_columns(self):
        ...
        assert "IF OBJECT_ID('tempdb..#PoleRawDataStaging')" in sql
        ...
        assert cols == pole_raw_data_loader._ALL_COLUMNS

    def test_staging_insert_placeholder_count_matches_all_columns(self):
        sql = pole_raw_data_loader._STAGING_INSERT_SQL
        assert sql.count("?") == len(pole_raw_data_loader._ALL_COLUMNS)
    ...
    def test_merge_from_staging_uses_intersect(self):
        assert "INTERSECT" in pole_raw_data_loader._MERGE_FROM_STAGING_SQL
    ...
    def test_merge_from_staging_insert_and_update_cover_all_non_key_columns(self):
        ...
        assert cols == set(pole_raw_data_loader._ALL_COLUMNS)

    def test_row_upsert_placeholder_count_matches_all_columns(self):
        sql = pole_raw_data_loader._ROW_UPSERT_SQL
        assert sql.count("?") == len(pole_raw_data_loader._ALL_COLUMNS)

    def test_sp_exec_id_excluded_from_diff_check_but_present_in_update(self):
        assert "SP_ExecId" not in pole_raw_data_loader._DIFF_CHECK_COLUMNS
        assert "SP_ExecId" in pole_raw_data_loader._NON_KEY_COLUMNS

    def test_retention_purge_uses_configured_month_count(self):
        sql = pole_raw_data_loader._RETENTION_PURGE_SQL
        assert f"-{pole_raw_data_loader.RETENTION_MONTHS}" in sql
    ...
    def test_missing_last_upload_sentinel_is_never_eligible_for_retention_purge(self):
        ...
        """
        sentinel_year = int(pole_raw_data_loader._MISSING_LAST_UPLOAD_SENTINEL[:4])
...
# --------------------------------------------------------------------------
# load_pole_raw_data() -- full flow
# --------------------------------------------------------------------------


class TestLoadPoleRawDataSuccessFlow:
    def test_full_success_flow_two_records(
        self,
        patch_get_connection_pole_raw_data,
        patch_fetch_lamps,
        mock_conn,
        mock_cursor,
        make_lamp_record,
        ...
        pole_raw_data_loader.load_pole_raw_data()
        ...
        assert (name, env, source) == ("loadPoleRawData", "Dev", "Leadsun")
        assert DTO_PATTERN.match(start_time)

        assert "CREATE TABLE #PoleRawDataStaging" in calls[1].args[0]
        assert "MERGE PoleRawData" in calls[2].args[0]
        assert calls[3].args[0] == "TRUNCATE TABLE #PoleRawDataStaging"
        assert "DELETE FROM PoleRawData" in calls[4].args[0]

        assert mock_cursor.executemany.call_count == 1
        staging_sql, batch = mock_cursor.executemany.call_args.args
        assert "INSERT INTO #PoleRawDataStaging" in staging_sql
        assert len(batch) == 2
    ...
    def test_empty_lamp_list_still_closes_out_execution_row(
        self, patch_get_connection_pole_raw_data, patch_fetch_lamps, mock_cursor
    ):
        ...
        assert len(calls) == 3
        assert "DELETE FROM PoleRawData" in calls[1].args[0]
    ...
    def test_records_missing_location_id_or_last_upload_are_counted_as_errors(
        self, patch_get_connection_pole_raw_data, patch_fetch_lamps, mock_cursor, make_lamp_record
    ):
    ...
    def test_null_last_upload_record_is_retained_not_dropped(
        ...
        assert batch[0][1] == pole_raw_data_loader._MISSING_LAST_UPLOAD_SENTINEL

    def test_retention_purge_logs_rowcount(
        self, patch_get_connection_pole_raw_data, patch_fetch_lamps, mock_cursor, caplog
    ):
        patch_fetch_lamps.return_value = []
...
class TestLoadPoleRawDataPartialFailure:
    def test_chunk_failure_falls_back_to_row_by_row(
        ...
        pole_raw_data_loader.load_pole_raw_data()  # must not raise
...
class TestLoadPoleRawDataTopLevelFailure:
    def test_fetch_failure_updates_error_message_and_reraises(
        self, patch_get_connection_pole_raw_data, patch_fetch_lamps, mock_conn, mock_cursor
    ):
        mock_cursor.fetchone.return_value = (11,)
