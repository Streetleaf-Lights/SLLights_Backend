...
Usage (from the Backend/ project root):
    ...
    python3 scripts/backfill_latest_hour_pole_vitals.py
...
run backfill_latest_hour_for_all_poles() itself, but importing
...
if __name__ == "__main__":
    # Without this, backfill_latest_hour_for_all_poles()'s
    ...
    from shared.pole_vitals_loader import backfill_latest_hour_for_all_poles

    logging.info(
        "Running latest-Hour-per-pole backfill (every pole's own most recent "
        "telemetry, regardless of age) against ENVIRONMENT=%s ...",
        environment,
    )
    backfill_latest_hour_for_all_poles()
    logging.info(
        "Backfill complete. Every pole with at least one PoleTelemetry reading "
        "now has an Hour PoleVitals row reflecting its own most recent known state."
    )
